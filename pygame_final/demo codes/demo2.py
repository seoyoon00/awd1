...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, platforms, playing):
        ...
        if self.rect != self.prev_rect:
            # Check if the player is on top of a platform
            on_platform = any(self.rect.colliderect(platform.rect) for platform in platforms)
            if not on_platform:
...
class Platform(pygame.sprite.Sprite):
    def __init__(self, x, y, height):
        ...
        self.spawn_delay = 100  # Adjust the spawn delay as needed
        self.last_spawn_time = pygame.time.get_ticks()

    def update(self):
        self.rect.y += 1

        # Move the platform to the top after a certain delay
        if pygame.time.get_ticks() - self.last_spawn_time > self.spawn_delay:
            ...
            new_platform = Platform(random.choice([100, 300, 500]), 0, random.choice([200, 300, 400, 500, 600]))
            list_of_platforms.add(new_platform)
...
# Create buttons
play_button = Button("Play", (screen_width // 2 - 100, screen_height // 2), action="play")
...
while running:
    ...
    if playing:
        ...
        if random.randint(0, 100) < 1:
            platform = Platform(random.choice([100, 300, 500]), 0, random.choice([200, 300, 400, 500, 600]))
        ...
        playing = player.update(list_of_platforms, playing)
